...
function Preview(props) {
  ...
  return <div className="bg-indigo-100 rounded-xl grow p-5 h-[85vh] max-w-[50%] overflow-y-hidden">
    <p className="font-semibold text-2xl py-3 ">{question || "what is your age"}?</p>
    ...
    <div className=" grow p-5 overflow-y-auto h-[85vh]">
      ...
      <div className="p-3">
        <div>
          ...
          <input onChange={(e) => setQuestion(e.target.value)} type="text" id="rounded-email" className=" rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" placeholder="What is your age"
            value={question || ""} />
        </div>
                ...
                pollInfo.map((item, index) => {
                  return (
                    <div className="flex relative flex-row-reverse" key={index}>
                      <span onClick={() => deleteOption(index)} className=" cursor-pointer rounded-r-md inline-flex  items-center px-3 border-t bg-white border-r border-b  border-gray-300 text-gray-500 shadow-sm text-sm">
                        <MdOutlineCancel />
                      </span>
        ...
        <div>
          ...
          <label htmlFor="file-upload" className="p-3 relative cursor-pointer bg-white rounded-md font-medium border border-gray-300 hover:border-gray-400 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-purple-500">
            <span className="inline-block py-2 px-4 rounded-md text-sm leading-5 font-medium text-gray-700">Choose a file</span>
            <input id="file-upload" name="file-upload" type="file" className="sr-only" />
          </label>
        </div>


        <button onClick={handleSavePoll} type="button" className="py-2 px-4 mt-6 bg-pink-600 hover:bg-pink-700 focus:ring-pink-500 focus:ring-offset-pink-200 text-white w-fit transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-full">
          Save
        </button>
...
function Polls() {
  ...
  return loading ? <Loader /> : (
    <div className="container m-auto flex w-full">
      ...
      <Modifications pollInfo={pollInfo} setPollInfo={setPollInfo} question={question} setQuestion={setQuestion} />
    </div>
  );
}
