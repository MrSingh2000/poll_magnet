  ...
  return loading ? (
    ...
    <div className="container w-full m-auto p-2 grid grid-cols-1 md:grid-cols-2 gap-4">
      {allPolls.map((item, index) => {
        let voted;
        if (!userDetails.email) voted = false;
        else voted = isAlreadyVoted(item, userDetails.userId);
            ...
            >
              <div className="flex justify-between">
                <p className="font-semibold text-2xl py-3">
                  {item.data.question}?
                  {voted && (
                    <span className="px-3 pb-1 text-xs rounded-full text-green-600 bg-green-200 relative bottom-4 left-2">
                      Voted
                    </span>
                  )}
                </p>
                <FaRegCopy
                  className="cursor-pointer bg-white rounded-full p-3 text-green-500 hover:text-white hover:bg-green-500 transition-all duration-500 delay-0"
                  size={50}
                  onClick={() => handleShareButton(item.id)}
                />
              </div>
                          ...
                          <>
                            <li key={pollItem.data[0].name + index}>
                              <input
                                type="radio"
                                name="selector"
                                id={pollItem.data[0].name + index}
                              />
                              <label
                                htmlFor={pollItem.data[0].name + index}
                                onClick={(e) => {
